...
def relevance_distribution(datasets=[None],
                           models=['shallow', 'deep', 'deep_v2', 'convdeep'],
                           model_path=None,
                           seqs=[12], use_sample=False, dry_run=False):
    ...
    if model_path is not None:
        ...
        datasets = [model_obj._.dataset]
        seqs = [model_obj._.seq_length]
        models = [model_obj]
    else:
        ...
        for m in models:
            ...
            for s in seqs:
                ...
                if model_path is None:
                    _path = _model_path(m, dataset, s)
                    model_obj = provider.load(_path)
                ...
                if not dry_run:
                    # if model_path is None:
                    #     file = './stats/rel-dist-%s-seq-%d-%s.pkl' % (dataset, s, m)
                    # else:
                    #     file =

                    with open('%s/rel-dist.pkl' % _path, 'wb') as fp:
