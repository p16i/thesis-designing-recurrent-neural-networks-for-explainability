...
Architecture = namedtuple('Architecture', ['in1', 'hidden', 'out', 'recur'])
...
class Dag(base.BaseDag):
    def __init__(self, no_input_cols, dims, max_seq_length, architecture, optimizer, no_classes):
        ...
        self.ly_output = Layer((architecture.hidden, architecture.out), 's3__final_output')
        ...
        # last_output_from_cell = tf.nn.relu(tf.matmul(ha_do, self.ly_output.W)
        #                                    - tf.nn.softplus(self.ly_output.b))

        # self.output_from_cell_activations = [last_output_from_cell]
        self.y_pred = tf.matmul(ha_do, self.ly_output.W)\
                      - tf.nn.softplus(self.ly_output.b)
...
class Network(base.BaseNetwork):
    ...
    def lrp(self, x, y, beta=0.0, alpha=1.0, debug=False):
        with self.get_session() as sess:
            ...
            # rel_to_output_from_cell = self.dag.layers['output'].rel_z_plus_prop(
            #     self.dag.outp_from_cell_activations[-1],
