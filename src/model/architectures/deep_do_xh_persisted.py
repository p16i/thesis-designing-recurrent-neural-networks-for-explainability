...
from model.components import tfx
...
class Dag(base.BaseDag):
    def __init__(self, no_input_cols, dims, max_seq_length, architecture, optimizer, no_classes):
        ...
        for i in range(0, max_seq_length, no_input_cols):
            ...
            itc_do, itc_do_mark = tfx.dropout_with_mark_returned(itc, keep_prob=self.keep_prob,
                                                                 binary_mark_tensor=itc_do_mark)
            ...
            rr, rr_do_mark = tfx.dropout_with_mark_returned(
                tf.matmul(ha, self.ly_recurrent.W) - tf.nn.softplus(self.ly_recurrent.b), keep_prob=self.keep_prob,
                binary_mark_tensor=rr_do_mark)
