...
class Dag(base.BaseDag):
    def __init__(self, no_input_cols, dims, max_seq_length, architecture, optimizer, no_classes):
        ...
        np.random.seed(71)
        mark = (np.random.uniform(0, 1, (architecture.hidden, architecture.recur)) < 0.2)*np.power(5, 0.5)
        mark2 = (np.random.uniform(0, 1, (architecture.recur, architecture.hidden)) < 0.2)*np.power(5, 0.5)
